...
namespace RatingControlSample.Controls
{
    ...
    public class RatingControl : TemplatedControl
    {
        ...
        public static readonly DirectProperty<RatingControl, int> HoverValueProperty =
        AvaloniaProperty.RegisterDirect<RatingControl, int>(
        nameof(HoverValue),
        o => o.HoverValue,
        (o, v) => o.HoverValue = v);
        ...
        {
            get => _hoverValue;
            set => SetAndRaise(HoverValueProperty, ref _hoverValue, value);
        }
        public static readonly StyledProperty<int> DisplayValueProperty =
            AvaloniaProperty.Register<RatingControl, int>(
                nameof(DisplayValue));

        public int DisplayValue => HoverValue > 0 ? HoverValue : Value;
        ...
        private void OnStarPointerEntered(object? sender, Avalonia.Input.PointerEventArgs e)
        {
            if (e.Source is Path star)
            {
                HoverValue = star.DataContext as int? ?? 0;
            }
        }
        private void OnStarPointerLeft(object? sender, Avalonia.Input.PointerEventArgs e)
        {
            if (e.Source is Path star && !_isInsideProperty)
            {
                HoverValue = 0;
            }
        }

        private void OnPropertyPointerEnter(object? sender, Avalonia.Input.PointerEventArgs e)
        {
            _isInsideProperty = true;
        }
        private void OnPropertyPointerLeft(object? sender, Avalonia.Input.PointerEventArgs e)
        {
            _isInsideProperty = false;
            HoverValue = 0;
        }
        ...
        protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
        {
            ...
            if (_starsPresenter is not null)
            {
                ...
                _starsPresenter.PointerEntered -= OnPropertyPointerEnter;
                _starsPresenter.PointerExited -= OnPropertyPointerLeft;
            }
            ...
            // This attaches events after initial load so we
            // don't get unwanted event for each star
            Dispatcher.UIThread.Post(() =>
            {
                AttachPointerHandlersToStars();
            }, DispatcherPriority.Loaded);

        }
        private void AttachPointerHandlersToStars()
        {
            if (_starsPresenter == null)
                return;

            foreach (var item in _starsPresenter.Items)
            {
                var container = _starsPresenter.ContainerFromItem(item);

                if (container != null)
                {
                    // Look for the Path inside the container
                    var path = container.GetVisualDescendants()
                                        .OfType<Path>()
                                        .FirstOrDefault();

                    if (path != null)
                    {
                        path.PointerEntered += OnStarPointerEntered;
                        path.PointerExited += OnStarPointerLeft;
                    }
                }
            }
        }
